...
public class AnchorDropDownRenderer extends JLabel implements ListCellRenderer {
  ...
  // This method is called every time a cell in the drop down box is to be rendered
    ...
    if (((String)value).equals(BaseElement.getAnchors()[0]))
      setIcon (null);
    else {
			// We only get the textual description so set that in the baseelement object
      el.setAnchor ((String)value);
			// Then we can get the number from the baseelement object
      setIcon (new ImageIcon (getClass().getResource(imageBase + "images/" + el.getAnchorImg())));
    }  
    return this;
  }
}